...
struct Args {
    #[arg(short, long, default_value = "wss://webrtc.nirbheek.in:8443")]
    server: String,
    #[arg(short, long)]
    peer_id: Option<u32>,
}
...
struct AppInner {
    ...
    send_msg_tx: Mutex<mpsc::UnboundedSender<Message>>,
}
...
impl App {
    ...
    > {
        ...
        "v4l2src ! autovideoconvert ! vp8enc deadline=1 ! rtpvp8pay pt=96 ! webrtcbin. \
        ...
        let (send_ws_msg_tx, send_ws_msg_rx) = mpsc::unbounded::<Message>();
...
async fn run(
    args: Args,
    ws: impl Sink<Message, Error = Error> + Stream<Item = Result<Message, Error>>,
) -> Result<(), anyhow::Error> {
