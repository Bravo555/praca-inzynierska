...
impl App {
    ...
    fn handle_websocket_message(&self, msg: &str) -> Result<(), anyhow::Error> {
        ...
        let json_msg: WebrtcMsg = serde_json::from_str(msg)?;
    ...
    fn on_offer_created(
        ...
        let message = serde_json::to_string(&WebrtcMsg::Sdp {
    ...
    fn on_ice_candidate(&self, mlineindex: u32, candidate: &str) -> Result<(), anyhow::Error> {
        let message = serde_json::to_string(&WebrtcMsg::Ice {
        ...
        let ws_msg = select! {
            ...
            ws_msg = ws_stream.select_next_some() => {
                ...
                match ws_msg? {
                    ...
                    WsMessage::Text(text) => {
                        app.handle_websocket_message(&text).context("bad websocket message")?;
                        None
                    },
...
async fn main_async() -> anyhow::Result<()> {
    ...
    // // TODO: add check if message is the session list
    // if let WsMessage::Text(session_list) = msg {
    //     println!("{session_list}");
    // }

    // if args.peer_id.is_none() {
    //     print!("join session: ");
    //     io::stdout().flush().unwrap();
    //     let mut session = String::new();
    //     io::stdin().read_line(&mut session)?;
    //     if !session.trim().is_empty() {
    //         args.peer_id.replace(session.parse()?);
    //     }
    // }
